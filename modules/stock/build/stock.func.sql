...
END;
$function$;
COMMENT ON FUNCTION stock.article_create(stock.article) IS 'Create article — INSERT + RETURNING';
...
END;
$function$;
COMMENT ON FUNCTION stock.article_delete(text) IS 'Delete (soft) article by id — sets active=false + RETURNING';
...
END;
$function$;
COMMENT ON FUNCTION stock.article_read(text) IS 'Read article by id — returns full row with FK resolved';
...
END;
$function$;
COMMENT ON FUNCTION stock.article_update(stock.article) IS 'Update article by id — partial update + RETURNING';
...
END;
$function$;
COMMENT ON FUNCTION stock.depot_create(stock.depot) IS 'Create depot — INSERT + RETURNING';
...
END;
$function$;
COMMENT ON FUNCTION stock.depot_delete(text) IS 'Delete (soft) depot by id — sets actif=false + RETURNING';
...
END;
$function$;
COMMENT ON FUNCTION stock.depot_read(text) IS 'Read depot by id — returns full row with article count';
...
END;
$function$;
COMMENT ON FUNCTION stock.depot_update(stock.depot) IS 'Update depot by id — partial update + RETURNING';

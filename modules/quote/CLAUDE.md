...
- Parameters via query string: `/devis?p_id=42` → `get_devis(p_id int)`
...
- UI primitives: `pgv.stat()`, `pgv.badge()`, `pgv.card()`, `pgv.grid()`, `pgv.empty()`, `pgv.md_table()`, `pgv.action()`, `pgv.toast()`, `pgv.redirect()`, `pgv.form()`, `pgv.input()`, `pgv.textarea()`, `pgv.sel()`, `pgv.accordion()`
...
- `{entity}_create(p_row quote.{entity}) RETURNS jsonb` — INSERT + RETURNING
- `{entity}_update(p_row quote.{entity}) RETURNS jsonb` — UPDATE (draft only) + RETURNING
- `{entity}_delete(p_id text) RETURNS jsonb` — DELETE (draft only) + RETURNING

Entities: `devis`, `facture`. Each has a matching `{entity}_ui()` for SDUI rendering.
...
3. Use `pgv.t('quote.xxx')` in nav_items(), brand(), and all get_*/post_* functions
4. `ON CONFLICT DO NOTHING` in the seed

## QA Seed Data

Schema `quote_qa` contains only `seed()` and `clean()` — NO pages.
...
- `pg_doc topic:sdui` — SDUI: ui_* primitives, _ui() convention, datasources

...
END;
$function$;
COMMENT ON FUNCTION cad._piece_tree_children(integer,integer) IS 'Build jsonb array of tree nodes for pieces and sub-groups under a parent group. Recursive.';
...
END;
$function$;
COMMENT ON FUNCTION cad.bill_of_materials(integer) IS 'Liste de debit: pieces groupees par role/section/essence avec volumes PostGIS. Sous-totaux par groupe.';
...
$function$;
COMMENT ON FUNCTION cad.brand() IS 'Brand name for cad module';
...
END;
$function$;
COMMENT ON FUNCTION cad.drawing_create(cad.drawing) IS 'Create drawing with default layer — returns jsonb of created record';
...
END;
$function$;
COMMENT ON FUNCTION cad.drawing_delete(text) IS 'Delete drawing by id (CASCADE layers, shapes, pieces) — returns jsonb of deleted record';
...
END;
$function$;
COMMENT ON FUNCTION cad.drawing_list(text) IS 'List drawings for current tenant with shape/piece counts — optional filter';
...
END;
$function$;
COMMENT ON FUNCTION cad.drawing_read(text) IS 'Read single drawing by id with enriched data (counts)';
...
END;
$function$;
COMMENT ON FUNCTION cad.drawing_update(cad.drawing) IS 'Partial update drawing by id — returns jsonb of updated record';
...
END;
$function$;
COMMENT ON FUNCTION cad.drawing_view() IS 'SDUI _view() template for drawing entity — 4 density levels + actions catalog';
...
END;
$function$;
COMMENT ON FUNCTION cad.duplicate_group(integer,real,real,real,text) IS 'Dupliquer un groupe entier (pieces + sous-groupes) avec offset de translation.';
...
END;
$function$;
COMMENT ON FUNCTION cad.fragment_piece_tree(integer) IS 'Fragment: tree explorer for 3D pieces with group hierarchy. Uses pgv.tree + cadPieceTree Alpine component.';
...
END;
$function$;
COMMENT ON FUNCTION cad.fragment_viewer(integer) IS 'Fragment: 3D viewer with raycaster selection, HUD, info panel, group info. Uses cadViewer Alpine component. Listens to tree events.';
...
END;
$function$;
COMMENT ON FUNCTION cad.get_drawing_bom(integer) IS 'Page liste de débit (BOM) avec navigation 2D/3D/BOM.';
...
END;
$function$;
COMMENT ON FUNCTION cad.group_pieces(integer,integer[],text) IS 'Grouper des pieces 3D ensemble. Retourne l id du groupe cree.';
...
END;
$function$;
COMMENT ON FUNCTION cad.i18n_seed() IS 'Seed French translations for CAD module into pgv.i18n';
...
END;
$function$;
COMMENT ON FUNCTION cad.inspect(integer) IS 'Vue complete du modele: pieces, groupes, bbox, volume, joints entre pieces.';
...
END;
$function$;
COMMENT ON FUNCTION cad.duplicate_piece(integer,double precision,double precision,double precision,text) IS 'Dupliquer une piece avec offset de translation. Preserve le group_id.';
...
END;
$function$;
COMMENT ON FUNCTION cad.list_groups(integer) IS 'Lister les groupes de pieces avec hierarchie, nombre de pieces et bbox.';
...
END;
$function$;
COMMENT ON FUNCTION cad.list_pieces(integer) IS 'Liste rapide des pieces avec position bbox et groupe.';
...
END;
$function$;
COMMENT ON FUNCTION cad.move_group_pieces(integer,real,real,real) IS 'Translater toutes les pieces d un groupe (recursif sur sous-groupes).';
...
END;
$function$;
COMMENT ON FUNCTION cad.nest_group(integer,integer) IS 'Imbriquer un groupe dans un autre (sous-assemblage). Detecte les cycles.';
...
END;
$function$;
COMMENT ON FUNCTION cad.remove_group(integer,boolean) IS 'Supprimer un groupe. keep_pieces=false supprime aussi les pieces, true les detache seulement.';
...
END;
$function$;
COMMENT ON FUNCTION cad.rename_group(integer,text) IS 'Renommer un groupe de pieces.';
...
END;
$function$;
COMMENT ON FUNCTION cad.render_wireframe(integer,text,integer,integer) IS 'Generate SVG wireframe projection of 3D model. Views: front (XZ), top (XY), side (YZ). Colored by role, labeled, with grid, coordinates and group bounding boxes.';
...
END;
$function$;
COMMENT ON FUNCTION cad.resize_piece(integer,double precision) IS 'Redimensionner une piece (reconstruit via add_beam). Preserve le group_id.';
...
END;
$function$;
COMMENT ON FUNCTION cad.ungroup_pieces(integer) IS 'Dissoudre un groupe: detache les pieces et sous-groupes, supprime le groupe.';

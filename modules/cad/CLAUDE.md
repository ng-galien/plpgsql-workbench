...
This module is an **independent module** of the pgView framework. Its dependencies are declared in `module.json`.
...
3. Use `pgv.t('cad.xxx')` in nav_items(), brand(), and all get_*/post_* functions
4. `ON CONFLICT DO NOTHING` in the seed

## QA Seed Data

The `cad_qa` schema contains only `seed()` and `clean()` — NO pages.
...
- Custom routing: `cad.page()` handles `/drawing/:id`, delegates the rest to `pgv.route()`

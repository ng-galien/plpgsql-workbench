...
BEGIN
  ...
  v_c.color_extra := '{"olive":"#5C6B3C","lavande":"#8B7EC8","pierre":"#D4C5A9"}'::jsonb;
  v_c.voice_personality := ARRAY['chaleureux','authentique','passionné'];
  v_c.voice_do := ARRAY['tutoyer','mots sensoriels','évoquer le terroir'];
  ...
  v_c.voice_personality := ARRAY['précis','minimaliste','technique'];
  v_c.voice_do := ARRAY['vouvoyer','vocabulaire technique','références architecturales'];

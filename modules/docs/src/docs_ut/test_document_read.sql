...
BEGIN
  ...
  v_jd := docs.document_create(jsonb_populate_record(NULL::docs.document, jsonb_build_object('name', 'Read Test', 'charte_id', v_jc->>'id')));
  ...
  RETURN NEXT is(v_j->>'charte_id', v_jc->>'id', 'charte_id');

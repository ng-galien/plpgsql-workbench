# planning -- Shared Agenda & Scheduling
...
3. Use `pgv.t('planning.xxx')` in nav_items(), brand(), and all functions
...
The `planning_qa` schema contains only `seed()` and `clean()` -- NO routing.

...
1. **Commercial chain**: CRM client → Quote devis (create, send, accept) → Facture (generate, send) → Ledger journal entry
2. **Purchase chain**: Purchase commande → Stock entree_reception → PMP recalculation
3. **Expense chain**: Expense note → validation → remboursement → Ledger entry
4. **Document chain**: Quote facture → Docs document_create → PDF export
5. **Planning chain**: Planning evenement with project.chantier FK → cross-module resolution
6. **HR chain**: Absence demande → validation → leave_balance decrement
...
4. Use Maket to document test results if needed (mcp__maket__doc_new)

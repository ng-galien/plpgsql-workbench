...
CREATE FUNCTION document.style_merge(p_existing text, p_new text) RETURNS text
...
CREATE FUNCTION document.xhtml_patch(p_html text, p_ops jsonb) RETURNS text
...
CREATE FUNCTION document.layout_check(p_html text, p_w numeric, p_h numeric) RETURNS text
...
CREATE FUNCTION document.charte_check(p_html text, p_charte_id text) RETURNS text
...
CREATE FUNCTION document.charte_tokens_to_css(p_charte document.charte) RETURNS text
...
Chaque fonction a un test pgTAP dans `document_ut` :

```sql
CREATE FUNCTION document_ut.test_charte_create() RETURNS SETOF text AS $$
BEGIN
...
- `pg_test schema:document_ut` pour valider
- `pg_pack schemas: document,document_ut,document_qa` pour exporter
- `pg_func_save target: plpgsql://document` pour versionner

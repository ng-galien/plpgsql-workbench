...
3. Use `pgv.t('project.xxx')` in nav_items(), brand(), and all functions
4. `ON CONFLICT DO NOTHING` in the seed

## QA Seed Data

Schema `project_qa` contains only `seed()` and `clean()` -- NO routing.
...
- Progress % per milestone -- situation billing = % x contract amount

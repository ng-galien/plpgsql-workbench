...
$function$;
COMMENT ON FUNCTION pgv.href(text) IS 'Build a route-aware link path. Reads pgv.route_prefix from session config.';
...
END;
$function$;
COMMENT ON FUNCTION pgv_ut.assert_page(text,text) IS 'Validate pgView HTML contract: data-rpc targets exist in pg_proc, href routes resolve to page_* functions, no inline styles, md blocks have valid table headers.';
...
END;
$function$;
COMMENT ON FUNCTION pgv_ut.test_href_with_prefix() IS 'Test pgv.href returns prefixed path when route_prefix is set';

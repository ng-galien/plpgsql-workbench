...
Real-time dashboard to monitor Claude Code agents. xterm.js terminals, modules, tests, hooks.

**Depends on:** `pgv`, `workbench`

**Schemas:** `ops`, `ops_ut` (tests)

**Note:** Messages and tools pages have been migrated to the `workbench` module. Ops focuses on agent monitoring: dashboard, modules, tests, health, agents/terminals, hooks, docs.
...
- `get_index()` → module landing page (required)
- Parameters via query string: `/agent?p_module=cad` → `get_agent(p_module text)`
...
- `pg_msg_inbox module:ops` → read incoming messages
- `pg_msg` → send a message to another module
- **feature_request / bug_report → ALWAYS via issue_report**: never send feature_request or bug_report directly. Create an issue: `INSERT INTO workbench.issue_report(issue_type, module, description) VALUES ('enhancement|bug', '<target_module>', '<description>')`. The lead will be notified and decide on dispatch.
...
1. Create `ops.i18n_seed()` — INSERT INTO pgv.i18n(lang, key, value) with translations
2. Namespaced keys: `ops.nav_xxx`, `ops.title_xxx`, `ops.btn_xxx`, etc.
3. Use `pgv.t('ops.xxx')` in nav_items(), brand(), and all get_*/post_* functions
4. `ON CONFLICT DO NOTHING` in seed

## QA Seed Data

The `ops_qa` schema contains only `seed()` and `clean()` — NO pages.
- `ops_qa.seed()` — INSERT realistic demo data
- `ops_qa.clean()` — DELETE in reverse FK order
- `ON CONFLICT DO NOTHING`, consider multi-tenant (`current_setting('app.tenant_id', true)`)

## Agent Workflow

1. On startup or when told "go": **always read `pg_msg_inbox module:ops`**
2. Process messages by priority (HIGH first)
3. Do not resolve a message until the task is verified
4. After each task: `pg_pack schemas: ops,ops_ut`
5. Then `pg_func_save target: plpgsql://ops` + `plpgsql://ops_ut`
...
- **You are the ops agent, NOT the lead.** Never use `ws_health` to find your tasks — it shows ALL workspace tasks. Use only `pg_msg_inbox module:ops` to read YOUR messages. Only process messages addressed to `ops`.
- Ops owns no tables — reads `workbench.agent_message`, `workbench.hook_log`, `workbench.agent_session`
- WebSocket on MCP port (3100), not PostgREST
- Persistent terminal — survives tab close
- xterm.js loads from CDN (no bundler)

...
Entities: `purchase_order`, `supplier_invoice`
...
3. Use `pgv.t('purchase.xxx')` in nav_items(), brand(), and all functions
4. `ON CONFLICT DO NOTHING` in the seed

## QA Seed Data

The `purchase_qa` schema contains only `seed()` and `clean()` -- NO routing.

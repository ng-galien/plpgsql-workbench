...
function topoSort(fns: FuncRow[], edges: Edge[]): FuncRow[] {
  ...
  const getRow = (rowKey: string): FuncRow => {
    const row = byKey.get(rowKey);
    if (!row) throw new Error(`missing function row for ${rowKey}`);
    return row;
  };
    ...
    for (const ck of calleeKeys) {
      for (const rk of callerKeys) {
        if (ck !== rk && keySet.has(ck) && keySet.has(rk)) {
          ...
          const outgoing = graph.get(ck);
          if (!outgoing) throw new Error(`missing graph node for ${ck}`);
          outgoing.push(rk);
        }
      }
    }
  ...
  queue.sort((a, b) => getRow(a).name.localeCompare(getRow(b).name));

  const result: FuncRow[] = [];
  while (queue.length > 0) {
    const k = queue.shift();
    if (!k) break;
    result.push(getRow(k));
    for (const caller of graph.get(k) ?? []) {
      const deg = (inDegree.get(caller) ?? 1) - 1;
      inDegree.set(caller, deg);
      if (deg === 0) {
        const callerName = getRow(caller).name;
        const idx = queue.findIndex((q) => getRow(q).name.localeCompare(callerName) > 0);
    ...
    handler: async (args) => {
      ...
      return withClient(async (client) => {
        ...
        for (const schema of schemas) {
          ...
          if (fns.length === 0) {
            ...
            for (const trig of triggers) {
              ...
              const triggerList = triggersByFunc.get(key);
              if (!triggerList) throw new Error(`missing trigger bucket for ${key}`);
              triggerList.push(
        ...
        for (const fn of sorted) {
          ...
          const schemaFns = dbFuncs.get(fn.schema);
          if (!schemaFns) throw new Error(`missing function set for schema ${fn.schema}`);
          schemaFns.add(fn.name);
        }

...
async function queryDeps(client: DbClient, schemas: string[]): Promise<Dep[]> {
  const hasPlpgsqlCheck =
    (await client.query(`SELECT 1 FROM pg_extension WHERE extname = 'plpgsql_check'`)).rowCount! > 0;
  ...
  for (const schema of schemas) {
    ...
    if (hasPlpgsqlCheck) {
      ...
      for (const fn of plFns) {
        try {
          ...
          const deps = await client.query(`SELECT type, schema, name FROM plpgsql_show_dependency_tb($1::oid)`, [
            fn.oid,
          ]);
    ...
    const astFns = hasPlpgsqlCheck ? fns.rows.filter((f) => f.lanname === "sql") : fns.rows;
...
function buildGraph(
  ...
  for (const schema of schemas) {
    const schemaFns = new Set(deps.filter((d) => d.source_schema === schema).map((d) => d.source));
    const tables = new Set(
      deps
        .filter((d) => d.source_schema === schema && d.dep_type === "RELATION" && d.target_schema === schema)
        .map((d) => d.target),
    );
...
export function createDocTool({ withClient }: { withClient: WithClient }): ToolHandler {
  return {
    metadata: {
      name: "pg_doc",
      description:
      ...
      schema: z.object({
        ...
        infra: z
          ...
          .optional()
          .describe(
            "Infrastructure schemas (comma-separated). Cross-schema calls to these are informational, not violations. Ex: pgv,workbench",
          ),
    ...
    handler: async (args, _extra) => {
      ...
      return withClient(async (client) => {
        ...
        const schemasSet = new Set(schemas);
        const crossSchema = deps.filter((d) => d.source_schema !== d.target_schema && d.dep_type === "FUNCTION");

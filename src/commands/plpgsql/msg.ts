    ...
    handler: async (args, _extra) => {
      ...
      return withClient(async (client) => {
        ...
        if (showSent) {
          ...
          const { rows } = await client.query<SentMessageRow>(
          ...
          const resolvedIds = rows
            .filter(
              (r) =>
                r.status === "resolved" &&
                r.resolved_at !== null &&
                (!r.acknowledged_at || r.resolved_at > r.acknowledged_at),
            )
            .map((r) => r.id);
        ...
        if (newIds.length > 0) {
          ...
          if (firstRow) {
            lines.push(`  pg_msg_inbox module:${mod} resolve:${firstRow.id} resolution:"..."`);
          }
        }
...
async function ensureTable(client: import("../../core/connection.js").DbClient): Promise<void> {
  await client.query(`

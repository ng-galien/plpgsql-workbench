...
export function createInspectStoreTool({ withClient }: { withClient: WithClient }): ToolHandler {
  return {
    metadata: {
      ...
      description: "Read the user's browser state: selection, phase, zoom. Reads from PG session table (synced by the browser).",
      schema: z.object({
    ...
    handler: async (args, _extra) => {
      return withClient(async (client) => {
        ...
        const lines = rows.map((r: any) => {
          const selected = Array.isArray(r.selected_ids) && r.selected_ids.length > 0
            ? r.selected_ids.map((id: string) => String(id).slice(0, 8)).join(", ")
            : "none";
          const ago = r.updated_at ? `(${Math.round((Date.now() - new Date(r.updated_at).getTime()) / 1000)}s ago)` : "";
...
export function createDispatchEventTool({ withClient }: { withClient: WithClient }): ToolHandler {
  return {
    metadata: {
      ...
      description: "Write to the session to simulate user actions (select element, change phase). The browser polls the session and reacts.",
      schema: z.object({
        canvas_id: z.string().describe("Canvas UUID"),
        selected_ids: z.array(z.string()).optional().describe("Set selection to these element IDs. Pass empty array to deselect."),
    ...
    handler: async (args, _extra) => {
      return withClient(async (client) => {
        ...
        await client.query(
          `UPDATE document.session SET ${sets.join(", ")} WHERE canvas_id = $1`,
          params,
        );
        return text(`Dispatched: ${args.selected_ids !== undefined ? `selected=[${(args.selected_ids as string[]).join(",")}]` : ""} ${args.phase ?? ""}`);
      });
    },
  };
}
...
export function createGetEventLogTool(_deps: { withClient: WithClient }): ToolHandler {
  return {
    metadata: {
      ...
      description: "Not available in PG-session mode. The event log is client-side only (Zustand store). Use ill_inspect_store to read current state.",
      schema: z.object({
        canvas_id: z.string().describe("Canvas UUID"),
      }),
    },

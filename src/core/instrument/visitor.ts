import { loadModule, parsePlPgSQL } from "@libpg-query/parser";
...
function walkStmt(stmt: Stmt, points: CoveragePoint[], pointId: () => string): void {
  ...
  for (const key of [
    ...
    if (stmt[key]?.lineno) {
      points.push({
        id: pointId(),
        line: stmt[key].lineno,
        kind: "block",
        label: key.replace("PLpgSQL_stmt_", ""),
        inject: "before",
      });
    }
  }
  ...
  if (stmt.PLpgSQL_stmt_if) {
    ...
    if (s.then_body?.length > 0) {
      ...
      if (firstLine)
        points.push({ id: pointId(), line: firstLine, kind: "branch", label: `IF true @${ifLine}`, inject: "before" });
    ...
    if (s.else_body?.length > 0) {
      ...
      if (firstLine)
        points.push({ id: pointId(), line: firstLine, kind: "branch", label: `ELSE @${ifLine}`, inject: "before" });
    ...
    if (s.exceptions?.PLpgSQL_exception_block?.exc_list) {
      for (const exc of s.exceptions.PLpgSQL_exception_block.exc_list) {
        ...
        if (e?.action?.length > 0) {
          ...
          const condNames = (e.conditions ?? []).map((c: Stmt) => c.PLpgSQL_condition?.condname ?? "?").join(", ");
          if (firstLine)
            points.push({
              id: pointId(),
              line: firstLine,
              kind: "branch",
              label: `EXCEPTION ${condNames}`,
              inject: "before",
            });

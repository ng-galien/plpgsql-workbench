 ...
 * Modules get module.json, build/, src/, frontend/, MCP config for dev DB.
 */
...
function rolesSql(_name: string): string {
  return `-- Roles (module schemas are created by their own DDL files)
  ...
  return `${JSON.stringify(
    {
      permissions: {
        allow: [
          ...
          "mcp__plpgsql-workbench__pg_func_rename",
          "mcp__plpgsql-workbench__pg_func_bulk_del",
          "mcp__plpgsql-workbench__pg_alter",
        ],
      },
...
export async function scaffoldModule(
  ...
  if (mode === "plx" && schemaName !== name) {
    throw new Error("PLX scaffold currently requires the public schema to match the module name");
  }
  ...
  const manifest = {
    ...
    sql:
      mode === "plx"
        ? [`build/${schemaName}.ddl.sql`, `build/${schemaName}.func.sql`, `build/${schemaName}_ut.func.sql`]
        : [`build/${schemaName}.ddl.sql`, `build/${schemaName}.func.sql`],
    assets: { frontend: [], scripts: [], styles: [] },
    grants: { anon: [schemaName] },
    ...(mode === "plx" ? { plx: { entry: `src/${name}.plx` } } : {}),
  };
  ...
  if (mode === "plx") {
    await fs.writeFile(
      path.join(moduleDir, "build", `${schemaName}_ut.func.sql`),
      `-- Auto-generated PLX unit tests from src/${name}.plx\n`,
    );
    created.push(`build/${schemaName}_ut.func.sql`);

    await fs.writeFile(path.join(moduleDir, "src", `${name}.plx`), plxStarter(name));
    created.push(`src/${name}.plx`);
  }
  ...
  // CLAUDE.md
  await fs.writeFile(path.join(moduleDir, "CLAUDE.md"), moduleClaudeMd(name, schemaName, description, mode));
  created.push("CLAUDE.md");

  return created;
}

function moduleClaudeMd(name: string, schema: string, description: string | undefined, mode: "sql" | "plx"): string {
...
6. JAMAIS editer \`build/*.func.sql\` — genere par \`pg_pack\``;
  const sourceLine =
    mode === "plx"
      ? `- \`src/${name}.plx\` → source PLX du module (contrat + fonctions + tests)`
...
- \`nav_items() -> jsonb\` → navigation du module. Retourne \`jsonb\` (JAMAIS TABLE)
...
1. Déclarer ses traductions dans \`src/${name}.i18n\`
2. Clés namespaced : \`${name}.nav_xxx\`, \`${name}.title_xxx\`, \`${name}.btn_xxx\`, etc.
3. Utiliser \`pgv.t('${name}.xxx')\` dans nav_items(), brand(), \`_view()\` et les helpers backend
4. Laisser \`plx_apply\` générer puis seed automatiquement \`${name}.i18n_seed()\`
...
function plxStarter(name: string): string {
  return `module ${name}
depends pgv

export fn ${name}.health() -> jsonb [stable]:

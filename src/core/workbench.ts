...
export async function resolveDoc(client: DbClient, topic: string): Promise<string> {
  ...
  const { rows } = await client.query<{ content: string }>(
    `SELECT content FROM workbench.doc WHERE topic = $1`,
    [topic],
  );
  if (rows.length === 0) return `doc "${topic}" not found`;
  return wrap(`plpgsql://workbench/doc/${topic}`, "full", rows[0].content, [
    `get plpgsql://workbench/doc`,
  ]);
}
...
export async function resolveDocIndex(client: DbClient): Promise<string> {
  ...
  const { rows } = await client.query<{ topic: string }>(
    `SELECT topic FROM workbench.doc ORDER BY topic`,
  );

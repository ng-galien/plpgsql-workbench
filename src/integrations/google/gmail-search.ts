...
export function createGmailSearchTool({ gmailClient }: { gmailClient: GmailClient }): ToolHandler {
  return {
    metadata: {
      name: "gmail_search",
      description:
    ...
    handler: async (args, _extra) => {
      ...
      for (const msg of messages) {
        ...
        const hasAttach = (detail.data.payload?.parts || []).some((p: any) => p.filename && p.filename.length > 0);

CREATE OR REPLACE FUNCTION sdui_ut.test_ui_form_for()
 RETURNS SETOF text
 LANGUAGE plpgsql
AS $function$
DECLARE
  ...
  f jsonb;
BEGIN
  v := sdui.ui_form_for('docs', 'charter');
  ...
  v := sdui.ui_form_for('docs', 'document');
  ...
  -- Verb override
  v := sdui.ui_form_for('docs', 'charter', 'patch');

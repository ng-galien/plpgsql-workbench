CREATE OR REPLACE FUNCTION sdui_ut.test_ui_card()
 RETURNS SETOF text
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  -- ui_card_header
  h := sdui.ui_card_header('◎', 'Test', sdui.ui_badge('VIP', 'success'));
  ...
  v := sdui.ui_stat('3', 'Pending', 'warn');
  RETURN NEXT is(v->>'variant', 'warn', 'stat: variant set');

  -- ui_card compact
  v := sdui.ui_card('crm://client/1', 'compact', sdui.ui_card_header('◎', 'Dupont'));
  ...
  v := sdui.ui_card(
    ...
    sdui.ui_card_header('🎨', 'Ocean'),
    jsonb_build_array(sdui.ui_stat('6', 'Couleurs')),
    jsonb_build_array(sdui.ui_card('docs://document/1', 'compact', sdui.ui_card_header('📄', 'Facture')))
  );
  ...
  v := sdui.ui_card(
    ...
    jsonb_build_array(sdui.ui_stat('6', 'Couleurs')),
    NULL,
    jsonb_build_array(sdui.ui_action('Supprimer', 'delete', 'docs://charte/ocean', 'danger', 'Confirmer ?'))
  );

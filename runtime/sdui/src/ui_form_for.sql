CREATE OR REPLACE FUNCTION sdui.ui_form_for(p_schema text, p_entity text, p_verb text DEFAULT 'set'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
AS $function$
...
BEGIN
  ...
  LOOP
    ...
    v_fields := v_fields || sdui.ui_field(v_rec.attname, v_field_type, v_label, v_required, v_options);
  END LOOP;

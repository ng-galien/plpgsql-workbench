...
BEGIN
  ...
  IF v_toolbar THEN
    v_html := v_html
      ...
      || '<button class="pgv-canvas-btn" id="' || v_id || '_reset" title="Taille reelle">1:1</button>'
      || '</div>';
  END IF;

  v_html := v_html || '</div>';

  -- Interaction JS (pan, zoom, fit)
  v_js := $JS$
(function(){
  var id="__ID__",
  ...
  if(!vp||!svg)return;
  var sc=1,mn=0.05,mx=10;
  function ap(){
    var bb=svg.viewBox.baseVal;
    if(bb.width){svg.style.width=(bb.width*sc)+"px";svg.style.height=(bb.height*sc)+"px";}
    if(zl)zl.textContent=Math.round(sc*100)+"%";
  }
  function fit(){
    var bb=svg.viewBox.baseVal;
    if(!bb.width)return;
    sc=Math.min(vp.clientWidth/bb.width,vp.clientHeight/bb.height)*0.95;
    ap();vp.scrollLeft=0;vp.scrollTop=0;
  }
  ...
  if(zi)zi.onclick=function(){sc=Math.min(mx,sc*1.25);ap();};
  if(zo)zo.onclick=function(){sc=Math.max(mn,sc/1.25);ap();};
  if(fb)fb.onclick=fit;
  if(rb)rb.onclick=function(){sc=1;ap();};
  vp.addEventListener("wheel",function(e){
    ...
    var r=vp.getBoundingClientRect(),
        ...
        rt=ns/sc;
    sc=ns;ap();
    vp.scrollLeft=px*rt-(e.clientX-r.left);
    vp.scrollTop=py*rt-(e.clientY-r.top);
  },{passive:false});
  ...
  addEventListener("mouseup",function(){if(dg){dg=false;vp.style.cursor="";}});
  fit();
})();
$JS$;

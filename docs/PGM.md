...
Copie les fichiers SQL et assets des modules dans le dossier de l'app. Si des assets frontend legacy existent encore, génère aussi `pgv-modules.js` comme loader transitoire.
...
#   pgv@1.0.0    pgView — fondation SDUI...
```
...
| `assets.scripts` | Scripts frontend optionnels (legacy ou shell client) |
| `assets.styles` | Styles frontend optionnels |
...
`pgm install` sait encore copier des assets frontend déclarés dans `module.json` et générer `pgv-modules.js`.
Ce mécanisme existe uniquement pour la période de transition du vieux shell HTML/JS vers le modèle SDUI.

Règle de design actuelle :
- ne pas introduire de nouvelles dépendances au shell HTML legacy
- considérer `pgv-modules.js` comme transitoire
- privilégier les contrats backend JSON/SDUI et un client dédié
...
src/pgm/
  ...
  installer.ts    # Copie fichiers + loader d'assets frontend legacy
